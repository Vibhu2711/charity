<?php
    ...
    if(isset($_POST['check_btn']))
    {
        ...
        // echo $extract_id; `request_status`='Approved'
		// $query = "UPDATE `donation_req_donor` SET `approv_time` WHERE `request_id` = $extract_id";
        $query = "UPDATE `donation_req_donor` SET `request_status` = 'Approved' WHERE `request_id` = $extract_id";
    ...
    if(isset($_POST['check_btn_rej']))
    {
        ...
        $query = "UPDATE `donation_req_donor` SET `request_status` = 'Rejected' WHERE `request_id` = $extract_id";
        $query_run = mysqli_query($con, $query);
...
<?php 
	...
	if(isset($_POST['submit'])) {
		...
		$sql = "INSERT INTO `feedback`(`request_id`, `N_email`, `D_email`, `item_detail`) 
                            VALUES ($r_id,'$nemail','$email','$message')";
		...
		<div class="container">
			<form method="post">
				<div class="row">
					...
					<?php
                        $query =  "SELECT * FROM `donation_req_donor` WHERE `request_id` = $extract_id ";
                        $query_run = mysqli_query($con, $query);
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<input type="text" name="rid" id="rid" class="form-control" value="<?=$extract_id ?>" placeholder="Request_id">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<input type="text" name="email" id="email" class="form-control" value="<?=$raw['email-id']?>" placeholder="Donor Email">
								</div>
							</div>
                            <div class="col-md-6">
								<div class="form-group">
									<input type="text" name="nemail" id="nemail" class="form-control" value="<?=$email?>" placeholder="NGO Email">
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									<textarea name="body" class="form-control" id="body" rows="7" cols="20" placeholder="Message">Item Detail: <?=$raw['item_detail']?></textarea>
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
