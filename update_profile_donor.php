	...
	<body>
        ...
        <?php
            ...
            if(isset($_POST['updated_profile']))
            {
                ...
                $update = mysqli_query($con, "UPDATE `donor_registration` SET `name` = '$update_uname', `surname` = '$update_sname', `email-id` = '$update_eid', `phone_no` = '$update_phone', `address` = '$update_add', `city` = '$update_city', `state` = '$update_state' WHERE `email-id` = '$email' ");
                if(isset($update))
                {
                    header("location: profile_donor.php");
                }
                else
                {
        ?>
