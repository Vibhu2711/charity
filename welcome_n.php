...
<html class="no-js"> <!--<![endif]-->
	...
	<body>
		...
		<form action="report.php">
			<br><br>
			<button type="submit" style="margin-left: 45%" class="btn">generate report</button>
			<br><br>
		</form> 
					...
					<form action="code.php" method="POST">
						<table class="table table table-bordered table-striped">
							<tbody>
								...
								<th style="width:10px; text-align:center;">D_id</th>
							...
							<tbody>
								...
								if(mysqli_num_rows($query_run) > 0)
								{
									foreach ($query_run as $raw) {
										...
										<tr>
											...
											<td style="width:10px; text-align:center;"><?= $raw['D_id'] ?></td>
								...
								else
								{
										?>
										<tr>
										<td collapse="7">No record Found</td>
										</tr>
										<?php
								}
								?>
						...
						<button name="btn_feedback" style="float: right;" type="submit" class="btn">Feedback</button>
					</form>
					</div>
				</div>
				</div>
					...
					<div class="card-body">
						...
						<table class="table table table-bordered table-striped">
							<tbody>
								...
								<th style="width:10px; text-align:center;">request_id</th>
								<th style="width:10px; text-align:center;">donation type</th>
								<th style="width:10px; text-align:center;">donor</th>
								<th style="width:10px; text-align:center;">request date</th>
								<th style="width:10px; text-align:center;">request status</th>
								<th style="width:10px; text-align:center;">Item Collection</th>
								</tr>
							...
							<tbody>
								<?php
								$query = "SELECT * FROM `donation_req_ngo` WHERE `email-id` = 'All' || `email-id` = '$email' ";
								// $query = "SELECT * FROM `donation_req_donor` WHERE `N_email` = 'All' || `N_email` = '$email' ";
								$query_run = mysqli_query($con, $query);
